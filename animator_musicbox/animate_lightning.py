...
donald_servo.angle = donald_last_pos
pluto_servo.angle = pluto_last_pos
mickey_servo.angle = mickey_last_pos
minie_servo.angle = minie_last_pos
goofy_servo.angle = goofy_last_pos
...
def animation(sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, file_name, num_pixels, garbage_collect):
    print(file_name)
    if file_name == "alien_lightshow":
        ...
        chips_birthday(sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, file_name, garbage_collect)
...
def fire_now_old(ledStrip, num_times, sleepAndUpdateVolume):
    ledStrip.brightness = 1.0
    r = 226
    g = 121
    b = 35
...
def moveToPositionGently (new_position, speed, sleepAndUpdateVolume, ledStrip, startTime, flashTime):
    global donald_last_pos
    global pluto_last_pos
    global mickey_last_pos
    global minie_last_pos
    global goofy_last_pos
    global flashTimeIndex
    sign = 1
    if donald_last_pos > new_position: sign = - 1
    for feller_angle in range( donald_last_pos, new_position, sign):
        timeElasped = time.monotonic()-startTime
        if timeElasped > flashTime[flashTimeIndex] - 0.25:
            ...
            elif 30.3125 == flashTime[flashTimeIndex]: #red
                ledStrip.fill((255, 0,0)) # r g b
                ledStrip.show()
            elif 30.8359 == flashTime[flashTimeIndex]: #orange
                ledStrip.fill((255, 165, 0)) # r g b
                ledStrip.show()
            elif 31.1386 == flashTime[flashTimeIndex]: #yellow
                ledStrip.fill((255, 234, 0)) # r g b
                ledStrip.show()
            elif 31.4414 == flashTime[flashTimeIndex]: #green
                ledStrip.fill((0, 255, 0)) # r g b
                ledStrip.show()
            elif 31.9844 == flashTime[flashTimeIndex]: #blue
                ledStrip.fill((0, 0, 255)) # r g b
                ledStrip.show()
            else:
                change_color(ledStrip)
            flashTimeIndex+=1
        ...
        sleepAndUpdateVolume(speed)
    donald_last_pos = new_position
    
def play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, file_name, garbage_collect):
    global flashTimeIndex
    flash_time_dictionary = files.read_json_file(file_name + ".json")
    flashTime = flash_time_dictionary["flashTime"]
    flashTimeLen = len(flashTime)
    flashTimeIndex = 0
    wave0 = audiocore.WaveFile(open(file_name + ".wav", "rb"))
    mixer.voice[0].play( wave0, loop=False )
    startTime = time.monotonic()
    while mixer.voice[0].playing:
        left_switch.update()
        ...
        if not left_switch.value:
            ...
            while mixer.voice[0].playing:
                pass
        pass
    ledStrip.fill((10, 10, 10)) # r g b
    ledStrip.show()
    garbage_collect("Song finished")
    return

def chips_birthday(sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, file_name, garbage_collect):
    if file_name == "July15":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/mickey_birthday_song", garbage_collect)
        return
    
    if file_name == "July16":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/you_got_a_friend_in_me", garbage_collect)
        return
    
    if file_name == "July17":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/when_you_wish_upon_a_star", garbage_collect)
        return

    if file_name == "July18":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/happy_birthday_in_the_park", garbage_collect)
        return
    
    if file_name == "July19":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/beauty_and_the_beast", garbage_collect)
        return

    if file_name == "July20":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/beegie_when_you_wish", garbage_collect)
        return
    
    if file_name == "July21":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/battle_hymn_jay", garbage_collect)
        return
    
    if file_name == "July22":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/some_day_my_prince", garbage_collect)
        return
    
    if file_name == "July23":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/ariel", garbage_collect)
        return
    
    if file_name == "July24":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/wishes", garbage_collect)
        return
    
    if file_name == "July25":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/reflections_of_earth", garbage_collect)
        return
    
    if file_name == "July26":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/small_world", garbage_collect)
        return
    
    if file_name == "July27":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/spectromagic", garbage_collect)
        return
    
    if file_name == "July28":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/aladin_i_can_show_you_the_world", garbage_collect)
        return
    
    if file_name == "July29":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/do_da", garbage_collect)
        return
    ...
    if file_name == "July31":
        play_song (sleepAndUpdateVolume, audiocore, mixer, ledStrip, left_switch, right_switch, "/sd/lightning_sounds/kiss", garbage_collect)
        return
